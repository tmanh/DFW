...
def train_epoch(model, dataloader, optimizer, device):
    model.train()
    loss_meter = []
    for xs, y_true, obs_mask in tqdm(dataloader, desc="Training"):
        xs = xs.to(device)          # (num_stations, num_features)
        y_true = y_true.to(device)  # (num_stations,)
        obs_mask = obs_mask.to(device)  # (num_stations,)

        # Forward: predict for all stations
        y_pred = model(xs)  # (num_stations,)

        # Only compute loss for masked stations (obs_mask == 0)
        mask = (obs_mask == 0)
        loss = nn.MSELoss()(y_pred[mask], y_true[mask])
...
def eval_epoch(model, dataloader, device):
    ...
    results = []
    with torch.no_grad():
        for xs, y_true, obs_mask in tqdm(dataloader, desc="Eval"):
            xs = xs.to(device)
