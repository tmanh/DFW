...
from model.p2w import sample_diffusion, train_diffusion
...
def test(cfg, train=True):
    ...
    if train:
        train_diffusion(model, train_loader, num_timesteps=1000, epochs=num_epochs, device=device)
    ...
    with torch.no_grad():
        for idx, (mx, _, my, _, _, mnrain, _, _) in enumerate(test_loader):
            ...
            for i in range(mx.shape[2] // seg_len):
                ...
                o = sample_diffusion(model, nrain, num_timesteps=1000, device=device)
