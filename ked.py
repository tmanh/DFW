...
from model.ked import KrigingExternalDrift
        ...
        for idx, (mxs, my, mlrain, mnbxs, mnby, mnrain, loc) in enumerate(test_loader):
            ...
            for i in range(my.shape[-1] // seg_len):
                ...
                for tidx in range(y.shape[1]):
                    model.fit(
                        nxs[0], nby[0, :, tidx], drift=nrain[0, :, tidx]
                    )

                    o[0, tidx] = model.predict(xs, lrain[0, tidx:tidx+1])[0]
